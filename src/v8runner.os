...
Функция ПутьКПлатформе1С(Знач Путь = "") Экспорт
	...
	Иначе
		...
		Если ЗначениеЗаполнено(мПутьКПлатформе1С) Тогда
...
// Возвращает, устанавливает значение переменной мПутьКПлатформе1С,
// если она не была определена ранее.
//
// Возвращаемое значение:
//   мПутьКПлатформе1С  - Путь к платформе 1С
//
Функция ПутьКПлатформе()

	ПутьКПредприятию_x64 = ПутьКПредприятию_8_3_x64();

	Если ЗначениеЗаполнено(ПутьКПредприятию_x64) Тогда
		мПутьКПлатформе1С = ПутьКПредприятию_x64;
	Иначе
		мПутьКПлатформе1С = ПутьКПредприятию_8_3_x32();
	КонецЕсли;
...
Функция ПутьКПредприятию_8_3_x64()

	ПутьКПредприятию_x64 = Платформа1С.ПутьКПредприятию("8.3", РазрядностьПлатформы.x64);
...
Функция ПутьКПредприятию_8_3_x32()

	ПутьКПредприятию_x32 = Платформа1С.ПутьКПредприятию("8.3", РазрядностьПлатформы.x32);

	Если ЗначениеЗаполнено(ПутьКПредприятию_x32) Тогда
		мПутьКПлатформе1С = ПутьКПредприятию_x32;
	КонецЕсли;
